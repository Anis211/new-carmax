  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        //setThumbnails((prev) => [
        //...prev,
        //{ id: item.ART_MEDIA_SUP_ID, src: item.ART_MEDIA_SOURCE },
        //])
        // );

        setMainImage(image[0].ART_MEDIA_SOURCE);
        setAnalogs(data.slice(0, 10));
      } catch (e) {
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mt-12 mb-12">
          ...
          <div className="flex flex-col gap-4">
            {analogs.map((part) => (
              <div
                key={part.ART_ARTICLE_NR}
                className="bg-gray-800 rounded-lg p-4 flex items-center justify-between"
              >
                <div className="flex-1">
                  <h3 className="text-lg font-bold">{part["PRODUCT GROUP"]}</h3>
                  <div className="text-gray-400 text-sm">
                    {part.ART_SUP_BRAND}
                  </div>
                </div>
                <div className="flex items-center gap-8">
                  <div className="text-yellow-500 font-bold text-lg">${0}</div>
                  <div className="flex items-center gap-2">
                    <button className="bg-gray-700 hover:bg-gray-600 text-white py-2 px-3 rounded-md flex items-center text-sm cursor-pointer !rounded-button whitespace-nowrap">
